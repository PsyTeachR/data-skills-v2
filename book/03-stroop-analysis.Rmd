...
* The weird `%>%` symbol is known as the *pipe* and it's loaded in as part of the `r pkg("tidyverse")`. The formal definition is that the pipe takes an object and sends it to the next function but it's maybe easier to read it as **and then**. So the below code reads "Take the object `ppt_infO` **and then** count it.
