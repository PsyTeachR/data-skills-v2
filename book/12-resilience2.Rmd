...
*There were `r dat_wide %>%count() %>% pull()` participants in total (mean age = `r mean(dat_wide$age, na.rm = TRUE)`, SD = `r sd(dat_wide$age, na.rm = TRUE)`, missing = `r missing_data$age[1]`)*
...
After participants were excluded for having missing data in the resilience scores, in total there were `r fitb(692)` participants (`r fitb(322)` men, `r fitb(348)` women, `r fitb(22)` non-binary, mean age = `r fitb(28.9)`, SD = `r fitb(6.60)`, missing = `r fitb(7)`). `r fitb(324)` participants were randomly assigned to the control condition and `r fitb(368)` were randomly assigned to the treatment condition.

Collapsing across both treatment conditions, the overall mean resilience score was `r fitb(3.03)` (SD = `r fitb(0.438)`, min = `r fitb(1.83)`, max = `r fitb(4.33)`). When analysed by treatment group, participants in the control condition  (mean = `r fitb(2.84)`, SD = `r fitb(0.391)`, min = `r fitb(1.83)`, max = `r fitb(3.83)`) self-reported `r mcq(c(answer = "lower", "higher"))` resilience scores than participants in the treatment condition (mean = `r fitb(3.19)`, SD = `r fitb(0.411)`, min = `r fitb(2)`, max = `r fitb(4.33)`).
