...
2. What is the name of the variable in `big5` that contains the Participant's ID? `r fitb("ResponseId")`.
...
4. How many questions are there in total on the Big 5 questionnaire? `r fitb(60)`
...
```{r echo = FALSE}
big5 %>%
  pivot_longer(cols = 2:61, names_to = "question_name", values_to = "answer") %>%
  head(15)
```
...
* What would be a better name for the variable `question_name`? `r fitb("item")`
* What would be a better name for the variable `answer`? `r fitb("response")`
