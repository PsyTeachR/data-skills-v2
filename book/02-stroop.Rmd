...
Additionally, if you get the message `could not find function "read_csv()"` is means that you have not loaded the `r pkg("tidyverse")` - a common error is to write the code but not run it!
:::
...
What is the **median** congruent reaction time to 1 decimal place? `r fitb(746.3)`

What is the **mean** incongruent reaction time to 1 decimal place? `r fitb(891.3)`
...
```{r, eval = FALSE, fig.cap - "Number of participants by gender"}
...
```{r, eval = FALSE, fig.cap= "Violin-boxplot of reaction times in each condition"}
