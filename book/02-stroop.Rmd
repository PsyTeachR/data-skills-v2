...
```{r img-upload, echo=FALSE, fig.cap="Uploading a zip file"}

# knitr::include_graphics("images/stroop/upload-zip.gif")
...
```{r fig.cap= "Participant info data"}
head(ppt_info)
```
...
As an example, let's load the `r pkg("beepr")` package which is already installed on the server. The `r pkg("beepr")` package is mostly just for fun - it makes R play a short noise - but it's useful to show you how packages work. To load a package, you use the function `library()` and include the name of the package you want to load in parentheses.
...
Now you can use the function `beep()`. A `r glossary("function")` is a name that refers to some code you can reuse.

* In code chunk 2, write and run the below code to use the function `beep()` (make sure you have your sound on).

```{r beepr-demo, eval = FALSE, filename="Using a function"}
beep()
```

After the function name, there is a pair of parentheses, which contain zero or more `r glossary("argument", "arguments")`. These are options that you can set. In the example above, the `sound` argument has a `r glossary("default value")` of `1`, which makes a "ping" sound. 
...
```{r beepr-demo-2, eval = FALSE, filename="Adding an argument"}
beep(sound = 8)
```
...
To use `r pkg("readr")` to import the data, we need to load the `r pkg("tidyverse")`.

* In code chunk 3, write and run the code to load the `r pkg("tidyverse")`. When you run this code, you're going to get something that at first glance might look like an error but it's not, it's just telling you which packages it has loaded.
...
Additionally, if you get the message `could not find function "read_csv()"` it means that you have not loaded the `r pkg("tidyverse")` - a common error is to write the code but not run it! To fix it, run the code that loads the tidyverse. Another reason you might see this message is if you've made a typo in the name of the function, so check that you've spelled `read_csv` exactly right.
:::
...
What is the **mean** overall reaction time to 1 decimal place? `r fitb(c(821.0, 821))`
