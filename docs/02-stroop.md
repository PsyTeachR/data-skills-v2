...
After the function name, there is a pair of parentheses, which contain zero or more <a class='glossary'>arguments<span class='def'></span></a>. These are options that you can set. In the example above, the `sound` argument has a <a class='glossary'>default value<span class='def'></span></a> of `1`, which makes a "ping" sound. 
